...
import kr.or.bit.service.ScheduleDeleteListService;
...
public class ScheduleController extends HttpServlet {
	...
	private void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	...
    	if(url_Command.equals("/list.Schedule")) {// 업무처리
			...
			forward.setPath("/WEB-INF/Page/ScheduleInsert.jsp");
		...
		}else if(url_Command.equals("/delok.Schedule")) {
		...
		}else if(url_Command.equals("/DeleteSchedule.Schedule")) {
			action = new ScheduleDeleteListService();
			try {
				forward = action.execute(request, response);
			} catch (Exception e) {
				System.out.println("스케줄 삭제리스트 업무 에러");
				e.printStackTrace();
			}
		}
